...
>
> ⚠ **The published picture and this app are mirror images of each other**, and only one of them is the real table. Computed, not eyeballed: the picture puts the attacker's deployment zone on the left and ours puts it on the right, at identical board `x`, so the two differ by a reflection rather than a rotation. The cause is the `y` convention — we plot `y` upward (the projection flips it once), the picture plots `y` downward. Nothing in the data settles which is right. It has never mattered because a bounding box is symmetric; it starts mattering the moment a wall goes on one side of a piece. **The recorded corners are unaffected** — they were derived by inverting the picture's own fitted transform, so they sit correctly relative to the zones and every other piece either way — but if the whole map is mirrored, so is every ruin on it.
>
