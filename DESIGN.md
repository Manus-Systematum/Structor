...
>
> **Two of the six ticks are now read off a published diagram, not guessed.** Battlemaster's own picture of `take-and-hold-vs-purge-the-foe-1` draws the walls, and one picture pins a part for good because the physical piece never changes. Tying the picture to the data was the careful part: a single affine map was fitted against **all six objective positions and both halves of the staggered deployment zones**, which is what rules out matching the wrong piece — the objectives alone are point-symmetric and fit two different mappings equally well. `EF` and `GH` are confirmed and recorded in `TerrainTemplate._wallCorner`; each appears twice in that layout at 180°, so the two placements cross-check. `AB`, `CD`, `Small L` and `Corner` are **not** recorded and keep the heuristic, because a wrong corner is worse than no corner: it states a wall position as confidently as a right one and the player sets their table out from it.
>
> ⚠ **The published picture and this app are mirror images of each other**, and only one of them is the real table — confirmed independently by the user against a build on their own device. Computed, not eyeballed: the picture puts the attacker's deployment zone on the left and ours puts it on the right, at identical board `x`, so the two differ by a reflection rather than a rotation. The cause is the `y` convention — we plot `y` upward (the projection flips it once), the picture plots `y` downward. **The evidence favours ours.** The library names three of its L polygons after the letter — `Short Corner L`, `Tiny Corner L`, `Corner Ruin (Left)` — and all three draw the letter **L** when `y` points up and a **Γ** when it points down; the `(Right)` variants come out as its mirror `┘` either way, which is the pair you would expect. That is inference from naming, not proof, and it is the only signal the data offers: winding order is mixed (CCW and CW within the same six templates) so it carries nothing, and no record declares an origin. **The recorded corners are unaffected either way** — they were derived by inverting the picture's own fitted transform, so they sit correctly relative to the zones and every other piece. Left as-is pending the picture's provenance; if it turns out to be Battlemaster's own rendering rather than a redraw, that outranks the naming argument and the projection should flip.
>
