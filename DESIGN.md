...
- **Bare caps** — a `max_count` over single items. Shown, coloured when exceeded, and **never used to refuse the tap**, because the reference list disproves them: a validated 2,000 point export carries four T'au flamers on a Commander whose record says `max_count: 3`.
  ...
  - ⚠ **An over-limit finding was written and then withdrawn**, and the reason belongs on the record. Making it a validation error flags the reference export — a validated 2,000 point list that prices to its printed total — as illegal in six places: four flamers against a cap of 3, two missile drones against 1, two gun drones against 1. The caps come from *both* sources, so this is not 40kdc's alone. Every option involved is `any_number: true`, which means the limit is per model rather than per unit, and applying it per unit is what produces the false positives — but a Commander is one model with four flamers, so scaling by model count does not rescue it either. The cap is good enough to shape a control and not good enough to call a list illegal, which is exactly what this section already said.
