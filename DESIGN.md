...
- Build number scheme: `version: <semver>+<build>`, currently `1.0.0+4`. App Store Connect refuses a build number it has already accepted, so the build number increments on every upload regardless of the version. `0.2.0` marked the release that added stratagems, secondaries and VP, the reference page and the army builder. `1.0.0` is the first version whose data reaches the thing a player reads: the correction channel (§3.15) publishes (§3.18), is named by content (§3.19), can be fetched on demand (§3.20), and lands on the snapshot an army is priced from (§3.22) rather than beside it.
